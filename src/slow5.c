...
// -1   null input
// -2   type invalid
int slow5_aux_meta_add(struct slow5_aux_meta *aux_meta, const char *attr, const char *type_str) {
    if (aux_meta == NULL || attr == NULL || type_str == NULL) {
        return -1;
    }

    enum aux_type type;
    int err = 0;
    type = str_to_aux_type(type_str, &err);
    if (err == -1) {
        return -2;
    }
