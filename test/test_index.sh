...
if [ "$zstd" = "1" ]; then
  ...
  TESTCASE_NO=5
  echo "------------------- slow5tools index testcase ${TESTCASE_NO} -------------------"
  $SLOW5_EXEC view $SLOW5_DIR/example_multi_rg_v0.2.0.blow5 -c zstd -s svb-zd -o $SLOW5_DIR/example_multi_rg_v0.2.0_zstd_svb-zd.blow5 || die "testcase ${TESTCASE_NO} failed"
