...
test/test_s2f_with_guppy.sh $DATA_NA12878/fast5 $TMP_DIR ./slow5tools guppy_basecaller || die "test_s2f_with_guppy failed"
rm -r $TMP_DIR

mkdir $TMP_DIR || die "Creating $TMP_DIR failed"
test/f2s_s2f_integrity_test.sh $DATA_NA12878/fast5 $TMP_DIR || die "f2s_s2f_integrity_test failed"
rm -r $TMP_DIR
