...
#reference fasta
MINIMAP=~/installs/minimap2/minimap2
SAMTOOLS=~/installs/samtools/samtools
...
prep () {
	...
	# #convert
    # multi_to_single_fast5 -i fast5/ -s single_fast5/ --recursive -t32

	# #fastt
	# /usr/bin/time -v $F5C_FASTT fastt single_fast5 > $FASTT 2> $folder/fastt.log
...
compile_code_noasync (){
	...
	cp -r ~/hasindu2008.git/f5c-fastt-async f5c-fastt-async
	cd f5c-fastt-async
