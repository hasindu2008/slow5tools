...
while [ "$i" -le "32" ]; do
    ...
    time -v "$SLOW5TOOLS_PATH" extract "-@$i" "$SLOW5_FILE" < "$READID_FILE" > "slow5.bench.out$i" 2> "slow5.bench.stderr$i"
    i=$((i*2))
done
...
while [ "$i" -le "32" ]; do
    clean_fscache
    echo "Extracting with $i threads"
    time -v "$SLOW5TOOLS_PATH" extract "-@$i" "$BLOW5_FILE" < "$READID_FILE" > "blow5.bench.out$i" 2> "blow5.bench.stderr$i"
    i=$((i*2))
done
...
while [ "$i" -le "32" ]; do
    clean_fscache
    echo "Extracting with $i threads"
    time -v "$SLOW5TOOLS_PATH" extract "-@$i" "$CLOW5_FILE" < "$READID_FILE" > "clow5.bench.out$i" 2> "clow5.bench.stderr$i"
    i=$((i*2))
done
