...
Skims through components in a SLOW5/BLOW5 file requested by user (using options) and prints to standard out. If no options are provided, all the SLOW5 records except the raw signal will be printed to standard out. enum data types are printed as strings.
