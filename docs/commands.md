...
*  `-c, --compress compression_type`:  
   Specifies the compression method used for BLOW5 output, i.e., SLOW5 output is uncompressed. `compression_type` can be `none` for uncompressed binary or `zlib` for zlib-based compression [default value: zlib]. Assumes `--to blow5`.
*  `-d, --out-dir STR`:  
